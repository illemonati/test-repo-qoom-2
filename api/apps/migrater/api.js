		...
		try {
			...
			for (const file of Object.keys(files)) {
				...
				const pullRequest = await octokit.createPullRequest({
					owner: currentUser.data.login,
					repo: repoName,
					title: `qoom-sync-${new Date().getTime()}`,
					body: `qoom-sync at ${new Date().toString()}`,
				    base: "master",
					head: `qoom-sync-${new Date().getTime()}`,
					changes: [
					  {
					    files: groupFiles,
					    commit:
					      `qoom-sync at ${new Date().toString()}`,
					  },
					],
				});
